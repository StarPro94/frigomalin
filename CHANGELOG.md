...
- 2026-08-06 23:12 — PWA : le service worker pouvait servir l'API en copie périmée — la garde réseau ne couvrait que les chemins commençant par `/api/` ; les routes `/api/recette`, `/api/inventaire` et `/api/chat` (sans slash final) passaient dans le cache d'abord → inventaire qui ne se rafraîchit plus sur les téléphones, recettes fantômes, erreurs hors-ligne masquées. La garde couvre désormais tout chemin commençant par `/api` et le cache est rafraîchi (v4). + déploiement Vercel (vérifié : `/` 200, `/api/recette` répond une recette).
